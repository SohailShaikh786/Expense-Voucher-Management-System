  ...
  return (
    <div className="page-shell">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-display text-2xl md:text-3xl font-semibold text-ink-900 tracking-tight flex items-center gap-2.5">
            <Clock className="w-6 h-6 text-amber-500" />
            Pending approval queue
          </h1>
          <p className="text-sm text-ink-500 mt-1">
            Claims awaiting your signature or rejection.
          </p>
        </div>
        <div className="px-4 py-2 bg-amber-50 border border-amber-200 rounded-xl text-xs font-bold text-amber-800">
          {vouchers.length} pending review
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="bg-white rounded-2xl border border-ink-200 p-8 space-y-4 animate-pulse">
          <div className="h-6 bg-ink-200 rounded w-1/4"></div>
          <div className="h-16 bg-ink-100 rounded"></div>
          <div className="h-16 bg-ink-100 rounded"></div>
        </div>
      ) : vouchers.length === 0 ? (
        <div className="bg-white rounded-2xl border border-ink-200 p-12 text-center shadow-sm">
          <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4">
            <FileCheck className="w-8 h-8" />
          </div>
          <h3 className="text-lg font-bold text-ink-800">Approval Queue is Clear!</h3>
        ...
        <div className="space-y-4">
          {vouchers.map((v) => (
            <div
              key={v.id}
              className="bg-white rounded-2xl border border-ink-200 shadow-xs hover:shadow-md transition-shadow p-5 sm:p-6"
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-4 text-sm">
                <div>
                  <span className="text-xs text-ink-400 uppercase font-semibold block">Employee</span>
                  <span className="font-bold text-ink-800">{v.employee?.name}</span>
                  <span className="text-xs text-ink-400 block">{v.employee?.email}</span>
                </div>
                <div>
                  <span className="text-xs text-ink-400 uppercase font-semibold block">Department & Category</span>
                  <span className="font-medium text-ink-800">{v.departmentName}</span>
                  <span className="text-xs text-ink-500 block">{v.expenseCategory.replace('_', ' ')}</span>
                </div>
                <div>
                  <span className="text-xs text-ink-400 uppercase font-semibold block">Expense Title</span>
                  <span className="font-medium text-ink-800 line-clamp-1">{v.expenseTitle}</span>
              ...
              {v.expenseDescription && (
                <p className="text-xs text-ink-600 bg-ink-50 p-3 rounded-xl border border-ink-100 mb-4">
                  {v.expenseDescription}
                </p>
              )}
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-ink-100">
                <Link
                  ...
                  className="text-xs font-semibold text-ink-600 hover:text-ink-900 flex items-center gap-1"
                >
            ...
            <form onSubmit={handleConfirmReject} className="p-6 space-y-4">
              <div>
                <p className="text-xs text-ink-500 mb-2">
                  ...
                  <strong className="text-ink-800">{activeRejectVoucher.voucherNumber}</strong>. The employee will see this explanation.
                </p>
                <textarea
                  rows={4}
                  required
